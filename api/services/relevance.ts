...
export interface RelevanceInputs {
  ...
  resumeText?: string | null; // when present, factor genuine skill overlap
}
...
export function scoreRelevance(job: RawJob, inputs: RelevanceInputs): number {
  ...
  // Role match is the strongest signal (weight 45).
  if (inputs.targetRole) {
    ...
    score += 45 * (0.7 * titleHit + 0.3 * bodyHit);
    weightUsed += 45;
  }

  // Industry match (weight 20).
  if (inputs.targetIndustry) {
    score += 20 * overlapScore(tokenize(inputs.targetIndustry), hay);
    weightUsed += 20;
  }

  // Extra keywords (weight 15).
  if (inputs.keywords && inputs.keywords.length) {
    const kw = inputs.keywords.flatMap(tokenize);
    score += 15 * overlapScore(kw, hay);
    weightUsed += 15;
  }

  // Genuine resume-skill overlap (weight 20): reward jobs whose requirements
  // this candidate actually meets, so ranking reflects real fit, not just
  // title word overlap. Only meaningful tokens from the resume are considered.
  if (inputs.resumeText && inputs.resumeText.trim().length > 40) {
    const resumeTokens = new Set(tokenize(inputs.resumeText));
    // Score how much of the JOB's meaningful vocabulary the resume covers.
    const jobTokens = [...new Set(tokenize(`${job.title} ${job.description}`))];
    const covered = jobTokens.filter((t) => resumeTokens.has(t)).length;
    const coverage = jobTokens.length ? covered / jobTokens.length : 0;
    // Normalize: even a strong resume rarely covers most JD tokens, so scale.
    score += 20 * Math.min(1, coverage * 2.5);
    weightUsed += 20;
  }
